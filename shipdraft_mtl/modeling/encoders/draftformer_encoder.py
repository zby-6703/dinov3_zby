...
"""Multi-scale deformable encoder component for DraftFormer."""
...
from shipdraft_mtl.modeling.common import Conv2d, ShapeSpec, get_norm
from shipdraft_mtl.utils.utils import _get_activation_fn, _get_clones

from .ms_deform_attn import MSDeformAttn
from .position_encoding import PositionEmbeddingSine

__all__ = ["DraftFormerEncoder"]
...
class DraftFormerEncoder(nn.Module):
    """Public DraftFormer encoder component."""
...
class _DraftFormerEncoderCore(nn.Module):
    """Multi-scale deformable encoder used by DraftFormer."""
    ...
    def __init__(
        ...
        *,
        ...
        input_shape_items = sorted(input_shape.items(), key=lambda item: item[1].stride)
        self.in_features = [name for name, _ in input_shape_items]
        self.feature_strides = [shape.stride for _, shape in input_shape_items]
        self.feature_channels = [shape.channels for _, shape in input_shape_items]
        self.feature_order = feature_order

        if feature_order == "low2high":
            transformer_input_shape = sorted(
                transformer_input_shape.items(),
                key=lambda item: -item[1].stride,
            )
        else:
            transformer_input_shape = sorted(
                transformer_input_shape.items(),
        ...
        self.transformer_in_features = [name for name, _ in transformer_input_shape]
        transformer_in_channels = [shape.channels for _, shape in transformer_input_shape]
        self.transformer_feature_strides = [shape.stride for _, shape in transformer_input_shape]
    ...
    def forward_features(self, features, masks):
        """Encode backbone features and build the FPN-style outputs."""
        srcs = []
        pos = []
        srcsl = []
        posl = []
