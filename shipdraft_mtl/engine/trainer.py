...
class Trainer:
    def __init__(self, cfg, mode="train", task="multitask"):
        ...
        mode = mode.lower()
        assert mode in ["train_eval", "train", "eval", "test"], "mode should be train, train_eval, eval or test"

        if self.cfg["Global"].get("device", "gpu") == "gpu" and torch.cuda.device_count() > 1 and "train" in mode:
        ...
        self.logger = get_logger(
            "shipdraft_mtl",
            os.path.join(self.cfg["Global"]["output_dir"], "train.log") if "train" in mode else None,
        )
        ...
        if "eval" in mode and self.cfg.get("Eval"):
            try:
                ...
                self.logger.info(f"valid dataloader build failed: {exc}")
        ...
        self.accumulation_steps = self.cfg["Global"].get("accumulation_steps", 1)
        self.optimizer, self.lr_scheduler = None, None
        step_each_epoch = len(self.train_dataloader) if self.train_dataloader is not None else 1
        epochs, _ = _resolve_train_limits(self.cfg["Global"], step_each_epoch=step_each_epoch)
    ...
    def train(self):
        ...
        if max_iter is not None and global_step >= max_iter:
            ...
            if self.writer is not None:
                self.writer.close()
            if torch.cuda.device_count() > 1 and dist.is_available() and dist.is_initialized():
        ...
        for epoch in range(start_epoch, epoch_num + 1):
            ...
            for idx, batch in enumerate(self.train_dataloader):
                ...
                loss = self._loss_for_backward(loss_dict) / self.accumulation_steps
                ...
                if is_main_process() and global_step > start_save_iter and global_step % save_iter_interval == 0:
                    save_ckpt(
                        self.model,
                        self.cfg,
                        self.optimizer,
    ...
    def eval_step(self, global_step, epoch):
        ...
        self.logger.info("cur metric, " + ", ".join(f"{key}: {value}" for key, value in cur_metric.items()))
        ...
        if cur_metric.get(indicator, 0.0) >= self.best_metric.get(indicator, 0.0):
